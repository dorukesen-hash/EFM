...
const GBP_OAUTH_DOC_PATH = { collection: 'settings', doc: 'googleBusinessOAuth' };

export async function getGbpOAuthConfig() {
  const db = admin.firestore();
  const ref = db.collection(GBP_OAUTH_DOC_PATH.collection).doc(GBP_OAUTH_DOC_PATH.doc);
  const snap = await ref.get();
  return snap.exists ? snap.data() : null;
}

export async function setGbpOAuthConfig(data) {
  const db = admin.firestore();
  const ref = db.collection(GBP_OAUTH_DOC_PATH.collection).doc(GBP_OAUTH_DOC_PATH.doc);
  await ref.set({
    ...data,
...
export async function clearGbpOAuthConfig() {
  const db = admin.firestore();
  const ref = db.collection(GBP_OAUTH_DOC_PATH.collection).doc(GBP_OAUTH_DOC_PATH.doc);
  await ref.set({
